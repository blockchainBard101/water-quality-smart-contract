...
public struct DayBucket has key {
    id: UID,
    day_utc: u64,                  
...
public fun create_device(name: String, clock: &Clock, ctx: &mut TxContext): Device {
...
public fun submit_reading_x100(
    ...
    let ts_ms_full = Clock::timestamp_ms(clock);
...
public fun latest(device: &Device): Option<SensorReading> {
    ...
    loop {
        ...
        if (Option::is_none(&device.first_day_utc) || day == Option::borrow(&device.first_day_utc).*) break
        else day = day - 1;
    };
...
public fun name(device: &Device): &string::String { &device.name }
